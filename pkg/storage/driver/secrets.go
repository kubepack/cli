...
var _ driver.Driver = (*ApplicationSecrets)(nil)

// SecretsDriverName is the string name of the driver.
const ApplicationSecretsDriverName = "ApplicationSecret"

// ApplicationSecrets is a wrapper around an implementation of a kubernetes
// SecretsInterface.
...
// NewApplicationSecrets initializes a new ApplicationSecrets wrapping an implementation of
// the kubernetes SecretsInterface.
func NewApplicationSecrets(ai cs.ApplicationInterface, si corev1.SecretInterface) *ApplicationSecrets {
...
func (d *ApplicationSecrets) Name() string {
	return ApplicationSecretsDriverName
}

// Get fetches the release named by key. The corresponding release is returned
// or error if not found.
func (d *ApplicationSecrets) Get(key string) (*rspb.Release, error) {
	// fetch the secret holding the release named by key
	obj, err := d.si.Get(context.Background(), key, metav1.GetOptions{})
	if err != nil {
...
func (d *ApplicationSecrets) List(filter func(*rspb.Release) bool) ([]*rspb.Release, error) {
	lsel := kblabels.Set{"owner": "helm"}.AsSelector()
	opts := metav1.ListOptions{LabelSelector: lsel.String()}

	list, err := d.si.List(context.Background(), opts)
	if err != nil {
...
func (d *ApplicationSecrets) Query(labels map[string]string) ([]*rspb.Release, error) {
	ls := kblabels.Set{}
...
func (d *ApplicationSecrets) Create(key string, rls *rspb.Release) error {
	...
	app, err := newApplicationObject(rls, lbs)
	if err != nil {
		return errors.Wrapf(err, "create: failed to encode release %q", rls.Name)
	}
	s, err := newSecretsObject(key, rls, lbs)
	if err != nil {
		return errors.Wrapf(err, "create: failed to encode release %q", rls.Name)
	}
	...
	if _, err := d.si.Create(context.Background(), s, metav1.CreateOptions{}); err != nil {
		if apierrors.IsAlreadyExists(err) {
			return driver.ErrReleaseExists
		}
...
func (d *ApplicationSecrets) Update(key string, rls *rspb.Release) error {
	// set labels for secrets object meta data
	var lbs labels
	...
	_, err = d.si.Update(context.Background(), s, metav1.UpdateOptions{})
	return errors.Wrap(err, "update: failed to update")
}

// Delete deletes the Secret holding the release named by key.
func (d *ApplicationSecrets) Delete(key string) (rls *rspb.Release, err error) {
	// fetch the release to check existence
	if rls, err = d.Get(key); err != nil {
		return nil, err
	}
	// delete the release
	err = d.ai.Delete(context.Background(), rls.Name, metav1.DeleteOptions{})
	if err != nil && !apierrors.IsNotFound(err) {
		return nil, err
	}
	err = d.si.Delete(context.Background(), key, metav1.DeleteOptions{})
	return rls, err
}
